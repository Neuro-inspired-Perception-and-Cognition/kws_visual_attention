...
def run_attention(window, net, device, resolution, num_pyr):
    ...
    # salmap = np.array((salmap - salmap.min()) / (salmap.max() - salmap.min()) * 255) <-- testing for investigating tau
    denom = salmap.max() - salmap.min()
    if denom == 0:
        salmap = np.zeros_like(salmap)
    else:
        salmap = np.array((salmap - salmap.min()) / denom * 255)
