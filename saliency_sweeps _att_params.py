...
# ATTENTION PARAMETERS SWEEP
def sweep_param(param_name, values, window_period=100, target_time_frac=0.5):
    """Run attention varying one ATTENTION_PARAMS key, holding all others fixed."""
    target_time = t.max() * target_time_frac
    maps, coords_list = [], []

    for val in values:
        params = copy.deepcopy(config.ATTENTION_PARAMS)
        params[param_name] = val
        net_attention = initialise_attention(device, params)  # fresh state per value

        time_cursor = window_period
        ...
        for xi, yi, pi, ti in zip(x, y, p, t):
            if ti <= time_cursor:
                ...
                saliency_map[:], salmax_coords[:] = run_attention(
                    window, net_attention, device, resolution, params['num_pyr']
                )
        ...
        maps.append(saliency_map.copy())
        coords_list.append(salmax_coords.copy())

    return maps, coords_list

# Ranges centered on your current config values (r0=7, rho=0.015, thick=12) — adjust as needed
param_sweeps = {
    'r0':    [3, 7, 12, 18, 25],
    'rho':   [0.005, 0.01, 0.015, 0.025, 0.04],
    'thick': [2, 6, 12, 20, 30],
}

fig, axes = plt.subplots(len(param_sweeps), 5, figsize=(20, 4 * len(param_sweeps)))

for row, (param_name, values) in enumerate(param_sweeps.items()):
    maps, coords_list = sweep_param(param_name, values)
    for col, (val, smap, coords) in enumerate(zip(values, maps, coords_list)):
        ax = axes[row, col]
        ax.imshow(smap, cmap='jet')
        ax.scatter(coords[1], coords[0], c='white', s=80, marker='x')
